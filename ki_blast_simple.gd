...
var animationName="113_ki_blasts_f_in_anm"
var animationName2="114_ki_blasts_f_loop_anm"
...
func _physics_process(delta: float) -> void:
	super._physics_process(delta)
		...
		#pass
	#else:
	if character.is_action_just_pressed("triangle") and !activo:
		iniciar()
...
func postIniciar():
	...
	pivot_suelto.global_position=character.characterPivot.global_position
	pivot_suelto.get_node("Node3D/personaje_glb/AnimationPlayer").play(animName,0.2)
	skeleton_fake=pivot_suelto.get_node("Node3D/personaje_glb/Armature/Skeleton3D")
	character.reiniciarBlendAnimationPlayer()
	desconectarSeñalAnimationPlayer(onAnimationFinished)
	character.animationPlayer.play(animName,0.2)
	...
	character.get_node("AnimationPlayer").play(animName)
